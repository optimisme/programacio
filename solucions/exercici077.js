...
async function main () {
    ...
    for (cnt = 0; cnt < propietatsPersona.length; cnt = cnt + 1) {
        propietat = propietatsPersona[cnt]
        console.log(`Propietat "${propietat}", valor: ${persona[propietat]}`)
    }

    // TODO: posa dins la variable 'propietatsAlumne' les propietats 
    //       de l'objecte 'alumne' en forma d'array fent servir 'Object.keys'
    propietatsAlumne = Object.keys(alumne)

    console.log(`Les propietats de l'objecte "alumne" són: ${propietatsAlumne}`)

    // TODO: fes un bucle que mostri els valors de cada propietat de persona
    for (cnt = 0; cnt < propietatsAlumne.length; cnt = cnt + 1) {
        propietat = propietatsAlumne[cnt]
        console.log(`Propietat "${propietat}", valor: ${alumne[propietat]}`)
    }
