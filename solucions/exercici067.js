...
async function main () {
    ...
    arxius = await llistarArxius('./')

    // Crea una carpeta anomenada 'copia' fent servir la funció 'crearCarpeta'
    // crida aquesta funció amb un 'await' al davant
    await crearCarpeta('copia')

    // Per cada un dels arxius de la carpeta actual
    ...
    for (cnt = 0; cnt < arxius.length; cnt = cnt + 1) {
        nom = arxius[cnt]
        if (nom.indexOf('exercici01') >=0 || nom.indexOf('exercici02') >=0) {
            await copiarArxiu(nom, 'copia/' + nom)
            console.log(`S'ha copiat ${nom}`)
        }
    }
